...
$("#goBtn").on("click", function (event) {
  ...
  var stofQuery =
    "https://api.stackexchange.com/2.2/search?order=desc&sort=activity&site=stackoverflow&tagged=" +
    tags +
    "&intitle=" +
    q;
  var mdnQuery =
    "https://developer.mozilla.org/en-US/search.json?locale=en-US&q=" +
    q +
    "%2B" +
    tags; //ajax query url's
  ...
  if (name == "") {
    ...
    $("#card-body2").append(
      "<div style='background-color: red; color:white;'>Please enter Name</div>",
    );
  } else if (role == "Check-in") {
    //if role isn't selected display error
    $("#card-body2").append(
      "<div style='background-color: red; color:white;'>Please select Student/Mentor</div>",
    );
  } else if (role == "Student") {
    //Change main div to fit student
    $("#card-body2").empty();
    $("#card-body2").append(
      "<button id='refresh' class='button btn-primary float-right'>New Question</button>",
    );
    ...
    $.ajax({
      ...
      for (var i = 0; i < 5; i++) {
        //create variables for necessary items
        ...
        var articleURL =
          ...
          "<a>";
        var newDiv = $(
          "<br><div class='card-text border border-dark rounded mx-auto'>",
        );
    ...
    $.ajax({
      ...
      for (var i = 0; i < 5; i++) {
        ...
        var docExcerpt =
          "'" + response.documents[i].excerpt.substr(0, 150) + "...'";
        var articleURL =
          "<a href='" +
          response.documents[i].url +
          "' target='_blank'>" +
          response.documents[i].url +
          "<a>";
        var newDiv = $(
          "<br><div id='articles' class='card-text border border-dark rounded mx-auto'>",
        );
        newDiv.append("<div id='articles'><b>" + docTitle + "</b></div>");
    ...
    idRef.on("child_added", function (snap) {
      if (snap.val().role == "Student") {
        ...
        var newIssue = $(
          "<br><div id='prob" +
            newUserID +
            "' class='card-text border border-dark rounded mx-auto'>",
        );
        newIssue.append(
          "<h4>" +
            snap.val().name +
        ...
        newIssue.prepend(
          "<button id='join" +
            newUserID +
            "' class='button btn-primary float-right joinButtons'" +
            " value='" +
            newUserID +
  ...
  $("#gif").empty();
  var gif1 = $("<img width=100% class='mb-2'>").attr(
    "src",
    "https://media.giphy.com/media/13f7pejozPXuta/giphy.gif",
  );
  var gif2 = $("<img width=100% class='mb-2'>").attr(
    "src",
    "https://media.giphy.com/media/v9fIPO9pbpdjG/giphy.gif",
  );
  var gif3 = $("<img width=100% class='mb-2'>").attr(
    "src",
    "https://media.giphy.com/media/gJZS10CemXPRm/giphy.gif",
  );
  $("#gif").append(gif1);
  $("#gif").append(gif2);
  $("#gif").append(gif3);
